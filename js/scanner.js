...
const Scanner = {
  ...
  async switchMode(mode) {
    ...
    // If AR tab clicked but AR is not supported, show explanation panel
    if (mode === 'ar') {
      const arTab = document.getElementById('scanTabAR');
      if (arTab?.dataset.arUnsupported === 'true') {
        // Update tabs visually
        document.querySelectorAll('.scanner-tab').forEach(t => t.classList.toggle('active', t.dataset.mode === 'ar'));
        document.querySelectorAll('.scanner-panel').forEach(p => p.classList.remove('active'));
        document.getElementById('scanPanel_ar').classList.add('active');
        // Show explanation — don't start AR session
